...
### 7.9 전역 보기 모드 (리스트/블로그/달력) — 전체화면 인수
- 콘텐츠 상단 우측에 **전역** `ViewModeToggle`(세그먼트 3버튼). `globalViewMode`(`DiaryListScreen` hoist)로 **화면 전체**를 전환.
- **리스트**: 기존 탭 대시보드(스탯카드 + 주간 스트립 + 탭바 기록/계획/목표/AI + 탭 콘텐츠).
- **블로그**: 탭·스탯·스트립 모두 숨기고 화면 전체가 **통합 피드** — 일기+계획+목표를 날짜 헤더로 묶어 최신순(`buildUnifiedByDate`, `DayItem`). 전체 기록(`state.allDiaryMetas`, 페이지네이션 무관).
- **달력**: 화면 전체 월간 7열 그리드(`buildMonthGrid`) + 도트(`unifiedDateSet` — 일기/계획/목표 무엇이든) + 선택일 하단 통합 목록(`unifiedForDate`). 월 이동, 외부 날짜 변경 동기화.
- 항목 렌더: 일기=`DiaryListItemCard`, 계획=`UnifiedTaskRow`(피치), 목표=`UnifiedGoalRow`(세이지).
- 리스트 탭의 기록 탭엔 타입 필터(전체/일기/새글/메모)만 유지(자체 보기 토글 제거 — 전역 토글로 대체).
