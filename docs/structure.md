...
| **다이어리 자물쇠 (v5.7)** | 4자리 비밀번호(PIN, SHA-256 해시) 보안 잠금. 앱 시작 및 백그라운드 복귀 시 비밀번호 확인. 설정 시 복구 불가 사전 경고 다이얼로그 제공 |
...
- **Google AdMob & AD_ID 권한 연동 (v4.6)**:
  ...
  - **화려하고 오묘한 오로라 글래스 테마 개편 (v5.6)**:
    - `AppTheme.AURORA_GLASS`("오로라 글래스", 🌌) 추가. 북극광처럼 매혹적이고 오묘한 일렉트릭 바이올렛(`#A855F7`), 네온 사이버 사안(`#06B6D4` / `#22D3EE`), 코스믹 핑크 글로우(`#F43F5E`), 프로스티드 라벤더 오팔 및 딥 코스믹 스페이스 미드나잇 배경으로 구현된 최상위 화려함의 오로라 글래스 테마 시스템 전면 구축.
  - **아틀라스(ATLAS) 기본 테마 가시성 & 고대비 개편 (v5.5)**:
    - 기존 탁했던 스틸 블루(`3D7BB5`)를 선명한 **로열 네이비 블루 (`#1D5D9B` / `#5C9CE6`)**로, 어두웠던 앰버를 **선버스트 앰버 골드 (`#D97706` / `#FBBF24`)**로 채도/명도 고도화. 본문 텍스트를 WCAG AAA 규격의 딥 슬레이트 블랙(`#0F172A`) 및 차콜(`#334155`)로 개선하여 화면 가독성 및 시각적 인지성 극대화.
